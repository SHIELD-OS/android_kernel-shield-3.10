...
static const signed char qmi_non_serial_ifaces[]  = { 8 , 10, 11};
...
static const struct usb_device_id id_table[] = {
	...
	/* Gobi 5K Boot VID/PID */
	{ USB_DEVICE(0x1199, 0x9040) },
	/* Gobi 5K Application VID/PID */
...
static DEVICE_ATTR(suspend_status, S_IRUGO, show_suspend_status,
							NULL);
...
static void sierra_indat_callback(struct urb *urb)
{
	...
	else 
	{
		if (urb->actual_length) 
		{
			...
			/* rx'd bytes */
			atomic_add(urb->actual_length,&intfdata->stats.rx_bytes);
			#if (LINUX_VERSION_CODE >= KERNEL_VERSION( 3,7,0 ))
...
static void sierra_dtr_rts(struct usb_serial_port *port, int on)
{
	...
	if (serial->dev) {
		sierra_lock_disc_mutex(serial);
		if (!serial->disconnected)
	sierra_send_setup(port);
		sierra_unlock_disc_mutex(serial);
	}
}
